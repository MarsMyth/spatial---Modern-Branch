...
public class Spatial implements ModInitializer {
    ...
    public void onInitialize() {
        // Register the payload type for S2C networking
        PayloadTypeRegistry.playS2C().register(SyncShapesPayload.ID, SyncShapesPayload.CODEC);
    ...
    private static void loadShapes(MinecraftServer server) {
        ...
        for (Map.Entry<String, String[]> entry : ConfigManager.config.shapeOverrides.entrySet()) {
            if (!Identifier.isNamespaceValid(entry.getKey())) {
                LOGGER.warn("Failed to load invalid override \"{}\" from config", entry.getKey());
                continue;
            }
            String[] parts = entry.getKey().split(":", 2);
            Identifier ID = parts.length == 2 ? Identifier.of(parts[0], parts[1]) : Identifier.of("minecraft", parts[0]);
    ...
    private static void loadDefaultOverrides() {
        ...
        addDyedShapeOverride(Identifier.ofVanilla("wool"), Items.WHITE_WOOL);
        // Other
    ...
    public static void addDyedShapeOverride(Identifier fromID, Item from) {
        for (DyeColor color : DyeColor.values()) {
            Identifier toID = Identifier.of(fromID.getNamespace(), String.format("%s_%s", color.getName().toLowerCase(), fromID.getPath()));
